...
/obj/item/weapon/aiModule/supplied/safeguard/attack_self(var/mob/user as mob)
	..()
	var/targName = sanitize(stripped_input(user, "Please enter the name of the person to safeguard.", "Safeguard who?", user.name,MAX_NAME_LEN))
	targetName = targName
...
/obj/item/weapon/aiModule/zeroth/oneHuman/attack_self(var/mob/user as mob)
	..()
	var/targName = sanitize(stripped_input(user, "Please enter the name of the person who is the only human.", "Who?", user.real_name,MAX_NAME_LEN))
	targetName = targName
...
/obj/item/weapon/aiModule/supplied/freeform/attack_self(var/mob/user as mob)
	...
	var/newlaw = ""
	var/targName = sanitize(stripped_input(user, "Please enter a new law for the AI.", "Freeform Law Entry", newlaw, MAX_MESSAGE_LEN))
	laws[1] = targName
