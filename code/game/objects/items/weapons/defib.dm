...
//poldan.sucks(dicks) && infamus.sucks(dicks)

/obj/item/weapon/cyborgpaddles/emp_act(severity)
	if(safety)
		safety = 0
		src.visible_message("<span class='notice'>[src] beeps: Safety protocols disabled!</span>")
		playsound(get_turf(src), 'sound/machines/defib_saftyOff.ogg', 50, 0)
...
/obj/item/weapon/cyborgpaddles/attack_self(mob/user)
	recharge(user)

/obj/item/weapon/cyborgpaddles/proc/recharge(mob/user)
	spawn(50)
		if(user && isrobot(user))
			if(user:cell:use(usecost))
				user.visible_message("<span class='notice'>[src] beeps: Unit ready.</span>")
				playsound(get_turf(src), 'sound/machines/defib_ready.ogg', 50, 0)
				cooldown = 0
			else
...
/obj/item/weapon/cyborgpaddles/update_icon()
	icon_state = "defibpaddles0"
	if(cooldown)
		icon_state = "defibpaddles0_cooldown"
	return

/obj/item/weapon/cyborgpaddles/attack(mob/M, mob/user)
	var/halfwaycritdeath = (config.health_threshold_crit + config.health_threshold_dead) / 2
	var/mob/living/carbon/human/H = M
	...
	if(busy)
		return
	if(cooldown)
		user << "<span class='warning'>Defibrillator is recharging.</span>"
		return
	if(!ishuman(M))
		user << "<span class='warning'>The instructions on Defibrillator don't mention how to revive that...</span>"
		return
	else
		if(user.a_intent == "harm" && !safety)
			...
			playsound(get_turf(src), 'sound/machines/defib_zap.ogg', 50, 1, -1)
			H.emote("gasp")
			add_logs(user, M, "stunned", object="cyborgpaddles")
			cooldown = 1
			busy = 0
			update_icon()
			recharge(user)
			...
			if(do_after(user, 30)) //beginning to place the paddles on patient's chest to allow some time for people to move away to stop the process
				...
				if(do_after(user, 20, target = src)) //placed on chest and short delay to shock for dramatic effect, revive time is 5sec total
					for(var/obj/item/carried_item in H.contents)
						if(istype(carried_item, /obj/item/clothing/suit/space))
							if(!combat)
								user.visible_message("<span class='warning'>Defibrillator buzzes: Patient's chest is obscured. Operation aborted.</span>")
								playsound(get_turf(src), 'sound/machines/defib_failed.ogg', 50, 0)
								busy = 0
								update_icon()
								return
					if(H.heart_attack)
						H.heart_attack = 0
						user.visible_message("<span class='notice'>Defibrillator pings: Patient's heart is now beating again.</span>")
					if(H.stat == 2)
						M.visible_message("<span class='warning'>[M]'s body convulses a bit.")
						playsound(get_turf(src), "bodyfall", 50, 1)
						playsound(get_turf(src), 'sound/machines/defib_zap.ogg', 50, 1, -1)
						...
						if(total_burn <= 180 && total_brute <= 180 && !H.suiciding && !ghost && tplus < tlimit && !(NOCLONE in H.mutations))
							...
							if (H.health > halfwaycritdeath)
								...
								H.adjustFireLoss((mobhealth - halfwaycritdeath) * (total_burn / overall_damage))
								H.adjustBruteLoss((mobhealth - halfwaycritdeath) * (total_brute / overall_damage))
							user.visible_message("<span class='notice'>Defibrillator pings: Resuscitation successful.</span>")
							playsound(get_turf(src), 'sound/machines/defib_success.ogg', 50, 0)
							H.stat = 1
							dead_mob_list -= H
							living_mob_list |= list(H)
							...
							if(tplus > tloss)
								H.setBrainLoss( max(0, min(99, ((tlimit - tplus) / tlimit * 100))))
							add_logs(user, M, "revived", object="cyborgpaddles")
						else
							if (H.suiciding || (NOCLONE in H.mutations))
								user.visible_message("<span class='warning'>Defibrillator buzzes: Resuscitation failed - Recovery of patient impossible. Further attempts futile.</span>")
							else if (tplus > tlimit)
								user.visible_message("<span class='warning'>Defibrillator buzzes: Resuscitation failed - Heart tissue damage beyond point of no return for defibrillation. Further attempts futile.</span>")
							else if(total_burn >= 180 || total_brute >= 180)
								user.visible_message("<span class='warning'>Defibrillator buzzes: Resuscitation failed - Severe tissue damage detected.</span>")
							else
								user.visible_message("<span class='warning'>Defibrillator buzzes: Resuscitation failed - No soul in patient body. Further attempts may be successful.</span>")
							playsound(get_turf(src), 'sound/machines/defib_failed.ogg', 50, 0)
						update_icon()
						cooldown = 1
						recharge(user)
					else
						user.visible_message("<span class='warning'>Defibrillator buzzes: Patient is not in a valid state. Operation aborted.</span>")
						playsound(get_turf(src), 'sound/machines/defib_failed.ogg', 50, 0)
			busy = 0
			update_icon()
		else
