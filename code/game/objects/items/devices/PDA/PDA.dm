...
//CHATROOM FUNCTIONS====================================
			...
			if("Set Nick")
				var/n = sanitize(trim(stripped_input(U, "Please enter nickname", name, nick, 9)))
				if(n)
					nick = n
...
/obj/item/device/pda/proc/create_message(var/mob/living/U = usr, var/obj/item/device/pda/P)
	...
	if(useMS && useTC) // only send the message if it's stable
		...
		tnote += "<i><b>&rarr; To [P.owner]:</b></i><br>[sanitize_russian(t, 1)]<br>"
		P.tnote += "<i><b>&larr; From <a href='byond://?src=\ref[P];choice=Message;target=\ref[src]'>[owner]</a> ([ownjob]):</b></i><br>[sanitize_russian(t, 1)]<br>"
		for(var/mob/M in player_list)
			if(isobserver(M) && M.client && (M.client.prefs.chat_toggles & CHAT_GHOSTPDA))
				M.show_message("<span class='game say'>PDA Message - <span class='name'>[owner]</span> -> <span class='name'>[P.owner]</span>: <span class='message'>[sanitize_russian(t)]</span></span>")
		...
		if(L)
			L << "\icon[P] <b>Message from [src.owner] ([ownjob]), </b>\"[sanitize_russian(t)]\" (<a href='byond://?src=\ref[P];choice=Message;skiprefresh=1;target=\ref[src]'>Reply</a>)"
...
/obj/item/device/pda/attackby(obj/item/C as obj, mob/user as mob, params)
	...
	else if(istype(C, /obj/item/weapon/screwdriver) && !src.wires)
		open = !open
		user << "<span class='notice'>You [open ? "open" : "close"] panel of [src.name].</span>"
		update_icon()
	else if(istype(C, /obj/item/stack/cable_coil))
		var/obj/item/stack/cable_coil/coil = C
		if(coil.use(5) && !src.wires && src.open)
			src.wires = 1
			user << "<span class='notice'>You add [C.name] to [src.name].</span>"
			update_icon()
	else if(istype(C,/obj/item/weapon/wirecutters) && src.wires)
		wires--
		new /obj/item/stack/cable_coil(get_turf(src),5)
		update_icon()
		user << "<span class='notice'>You remove wires from [src.name].</span>"
	return

/obj/item/device/pda/update_icon()
	if(src.open)
		src.overlays += image('icons/obj/pda.dmi', "pda-open")
	if(src.wires)
		src.overlays += image('icons/obj/pda.dmi', "pda-cable")
...
/obj/item/device/pda/proc/make_beep()
	if (!silent)
