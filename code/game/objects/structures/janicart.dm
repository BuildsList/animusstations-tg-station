...
/obj/structure/stool/bed/chair/janicart/invalidpimp
	name = "invalid pimp"
	desc = "That invalid pimp."
	icon = 'icons/obj/vehicles.dmi'
	icon_state = "invalid"

/obj/structure/stool/bed/chair/janicart/invalidpimp/Move()
	..()
	playsound(src.loc, 'sound/machines/copier.ogg', 70, 1)

/obj/structure/stool/bed/chair/janicart/invalidpimp/relaymove(mob/user, direction)
	if(user.stat || user.stunned || user.weakened || user.paralysis)
		unbuckle_mob()
	else
		if(!Process_Spacemove(direction) || !has_gravity(src.loc) || move_delay)
