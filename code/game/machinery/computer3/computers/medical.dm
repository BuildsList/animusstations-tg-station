...
/datum/file/program/med_data
	...
	//							//Foreach goto(132)
		...
		//usr << browse(text("<HEAD><TITLE>Medical Records</TITLE></HEAD><TT>[]</TT>", dat), "window=med_rec")
		//onclose(usr, "med_rec")
		var/datum/browser/popup = new(usr, "med_rec", "Medical Records Console", 600, 400)
...
/datum/file/program/med_data/proc/canUseMedicalRecordsConsole(mob/user, message = 1, record1, record2)
	if(user)
		if(message)
			if(authenticated)
