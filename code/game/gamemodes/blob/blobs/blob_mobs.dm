...
/mob/living/simple_animal/hostile/blob/blobspore/proc/request_player()
...
/mob/living/simple_animal/hostile/blob/blobspore/proc/question(var/client/C)
	spawn(0)
		if(!C)	return
		var/response = alert(C, "A blob spore needs a player. Are you interested?", "Blob spore request", "Yes", "No", "Never for this round")
		if(!C || ckey)
			return
		if(response == "Yes")
...
/mob/living/simple_animal/hostile/blob/blobspore/proc/transfer_personality(var/client/candidate)
	...
	if(src.mind)
		log_admin("New blob spore player: ", src.ckey)
