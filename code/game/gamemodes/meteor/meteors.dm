...
/proc/spawn_meteor_at_location(var/turf/pickedstart,var/turf/pickedgoal,var/meteortype, z_origin)
	usr << "ok spawn [meteortype] at [pickedstart.loc] to [pickedgoal.loc]"
	var/obj/effect/meteor/M = new meteortype(pickedstart)
	M.dest = pickedgoal
	M.z_original = z_origin
	spawn(0)
		usr << "ok go to walK_towards()"
		walk_towards(M, M.dest, 1)
		usr << "ok stop walk_towards()"
	usr << "ok return"
	return
