...
/mob/dead/observer/proc/singulo()
	var/count = 0
	for(var/mob/dead/observer/G in get_turf(src))
		count++
	if(count > config.ghosts_need_to_fucking_singulo)
		var/obj/singularity/S = new /obj/singularity(get_turf(src))
		S.energy = 500
		config.ghosts_need_to_fucking_singulo += 20
