...
/datum/admins/proc/makeDeathsquad()
	var/mission = sanitize(input("Assign a mission to the deathsquad", "Assign Mission", "Leave no witnesses."))
...
/datum/admins/proc/makeOfficial()
	var/mission = sanitize(input("Assign a task for the official", "Assign Task", "Conduct a routine preformance review of [station_name()] and its Captain."))
...
/datum/admins/proc/makeEmergencyresponseteam()
	...
	switch(alert)
		...
		if("Green: Centcom Official")
			makeOfficial()
			return
	var/teamsize = min(8,input("Maximum size of team? (8 max)", "Select Team Size",5) as null|num)
		...
		while(numagents && candidates.len)
			...
			switch(numagents)
				...
				if(7)
					...
					equip_emergencyresponsesquad(ERTOperative, "eng",redalert)
				if(7)
					ERTOperative.real_name = "Support [pick(lastname)]"
					equip_emergencyresponsesquad(ERTOperative, "cha",redalert)
