...
/obj/effect/bmode/buildhelp/Click()
	switch(master.cl.buildmode)
		...
		if(5)
			usr << "\blue ***********************************************************"
			usr << "\blue Left Mouse Button on turf/obj/mob  make ex_act()"
			usr << "\blue Right Mouse Button on buildmode icon to change power of ex_act()"
			usr << "\blue ***********************************************************"
	return 1
...
/obj/effect/bmode/buildmode/Click(location, control, params)
	...
	if(pa.Find("left"))
		...
		switch(master.cl.buildmode)
			...
			if(5)
				master.buildmode.kaboompower = input(usr, "Enter value of power:", "Value", 3) as num

				if(master.buildmode.kaboompower >= 3)
					master.buildmode.kaboompower = 3.0
				else if(master.buildmode.kaboompower <= 1)
					master.buildmode.kaboompower = 1.0
				else master.buildmode.kaboompower = 2.0
			if(6)
				meteor_type = input(usr,"Select Meteor Size","Meteor",null) as anything in list(/obj/effect/meteor/medium,/obj/effect/meteor/big,/obj/effect/meteor/flaming,
				/obj/effect/meteor/irradiated, /obj/effect/meteor/meaty,/obj/effect/meteor/meaty/xeno,/obj/effect/meteor/tunguska,
...
/proc/build_click(var/mob/user, buildmode, params, var/obj/object)
	...
	switch(buildmode)
		...
		if(5)
			if(pa.Find("left"))
				object.ex_act(holder.buildmode.kaboompower)
				log_admin("Build Mode: [key_name(usr)] make ex_act([holder.buildmode.kaboompower]) at [object] ([object.x],[object.y],[object.z])")
		if(6)
			if(pa.Find("left"))
				world << "ok clicked left"
				if(object.loc && holder.loc)
					world << "ok have loc"
					spawn_meteor_at_location(holder.loc, object.loc, holder.buildmode.meteor_type)
				else
					usr << "fail loc a object= [object.loc] a holder = [holder.loc]"



			if(pa.Find("right"))
				world << "ok clicked right"
				if(object.loc && user.loc)
					world << "ok have loc object x/y = [object.x]/[object.y]"
					spawn_meteor_at_location(user.loc, object, holder.buildmode.meteor_type, user.z)
				else
					usr << "fail loc a object= [object.loc] a user = [user.loc]"