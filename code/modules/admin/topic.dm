/datum/admins/Topic(href, href_list)
	...
	/////////////////////////////////////new ban stuff
		...
		if(appearance_isbanned(M))
			...
			if("Yes")
				var/reason = sanitize(input(usr,"Reason?","reason","Trap") as text|null)
				if(!reason)
					return
...
*/
	...
	//Player Notes
		...
		var/sound/S = sound('sound/effects/now_go_to_the_forums_and_cry_like_a_little_bitch_you_are.ogg')
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
		...
		if(!check_rights(R_ADMIN))	return
		usr << "DEBUG: [src.owner.mob] && [src.owner.mob.ckey] && [src.owner]"
		...
		var/speech = sanitize(input("What will [key_name(M)] say?.", "Force speech", ""))// Don't need to sanitize, since it does that in say(), we also trust our admins.
		if(!speech)	return
		M.say(speech)
