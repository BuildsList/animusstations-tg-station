...
/obj/machinery/power/apc/attackby(obj/item/W, mob/living/user, params)
	...
	if (istype(W, /obj/item/weapon/crowbar) && opened)
		...
		if(emagged)
			...
			if(istype(W, /obj/item/device/pda) && locked)
				...
				if(P.wires && !P.in_use)
					...
					for(var/i = 1, i < 7, i++)
						if(do_after(user, 100, target = src))
							counter++
							P.make_beep()
							user << "<span class='notice'>Subsystem [i]/6 reloaded.</span>"
						else counter = 100
					if(counter >= 6 && counter != 100)
						locked = !locked
						P.in_use = 0
						user << "<span class='notice'>You successful reboot the [src.name] interface.</span>"
					else if(counter == 100)
						P.in_use = 0
						user << "<span class='warning'>You cancel rebooting [src.name]!</span>"
				P.in_use = 0
			else
