...
//some autodetection here.
	...
	"borer" = /datum/game_mode/borer				//15
)
	...
	proc/process_link(mob/user, list/href_list)
		...
		switch(href_list["task"])
			...
			if("input")
				switch(href_list["preference"])
					if("ghostform")
						var/new_form = input(user, "Choose your ghostly form:","BOO!",null) as null|anything in ghost_forms
						if(new_form)
							ghost_form = new_form
					if("name")
						var/new_name = sanitize( reject_bad_name( input(user, "Choose your character's name:", "Character Preference")  as text|null ))
						if(new_name)
							real_name = new_name
						else
