...
/obj/machinery/computer/helm/proc/update_link()
...
/obj/machinery/computer/helm/ui_interact(mob/user, ui_key = "main")
	if(!linked || user.stat)
		return
	...
	data["d_x"] = dx
	data["d_y"] = dy
	data["speed"] = linked.get_speed() ? linked.get_speed() : 0
	...
	if (!ui)
		ui = new /datum/nanoui(user, src, ui_key, "helm.tmpl", "[linked.name] Helm Control", 380, 530)
