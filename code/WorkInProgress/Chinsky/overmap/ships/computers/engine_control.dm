...
/obj/machinery/computer/engines/proc/update_link()
...
/obj/machinery/computer/engines/ui_interact(mob/user, ui_key = "main")
	if(!linked)
		return
	...
	if (!ui)
		ui = new(user, src, ui_key, "engines_control.tmpl", "[linked.name] Engines Control", 410, 530)
...
/obj/machinery/computer/engines/soviet/attackby(obj/item/P as obj, mob/user as mob)
	if(stat & (NOPOWER|BROKEN))
		return
	if(istype(P, /obj/item/device/multitool) && error)
