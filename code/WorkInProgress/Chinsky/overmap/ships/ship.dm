...
/obj/effect/map/ship/New(var/obj/effect/mapinfo/data)
	...
	if (data.icon != 'icons/mob/screen_gen.dmi')
		icon = data.icon
...
/obj/effect/map/ship/process()
	if(!is_still())
		...
		if(newloc)
			Move(newloc)
	//	if(prob(3))
	//		spawn_meteor_at_location(locate(rand(40,world.maxx-40), world.maxy-15), locate(rand(40,world.maxx-40), 15), meteors_normal)
			// Если он и появится, то скорей всего не встретится с судном